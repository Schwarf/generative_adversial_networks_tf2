...
dataset, metadata = tensorflow_datasets.load(
...
def random_crop(image):
    cropped_image = tensorflow.image.random_crop(image, size=[IMG_HEIGHT, IMG_WIDTH, 3])
...
def random_jitter(image):
    ...
    image = tensorflow.image.resize(
        image, [286, 286], method=tensorflow.image.ResizeMethod.NEAREST_NEIGHBOR
    )
...
loss_obj = tensorflow.keras.losses.BinaryCrossentropy(from_logits=True)


def discriminator_loss(real, generated):
    real_loss = loss_obj(tensorflow.ones_like(real), real)

    generated_loss = loss_obj(tensorflow.zeros_like(generated), generated)
...
def generator_loss(generated):
    return loss_obj(tensorflow.ones_like(generated), generated)


def calc_cycle_loss(real_image, cycled_image):
    loss1 = tensorflow.reduce_mean(tensorflow.abs(real_image - cycled_image))

    return LAMBDA * loss1


def identity_loss(real_image, same_image):
    loss = tensorflow.reduce_mean(tensorflow.abs(real_image - same_image))
...
ckpt = tensorflow.train.Checkpoint(
...
ckpt_manager = tensorflow.train.CheckpointManager(ckpt, checkpoint_path, max_to_keep=5)
...
def generate_images(model, test_input, epoch):
    ...
    for i in range(2):
        ...
        plt.axis("off")
    f.savefig("generator_output" + str(epoch) + ".png")
...
for epoch in range(EPOCHS):
    ...
    n = 0
    for image_x, image_y in tensorflow.data.Dataset.zip((train_horses, train_zebras)):
